...
import { assertNotHumanGate, type StatusFetcher } from "../../src/security/gate_guard.js";
import { ERR } from "../../src/types.js";
import type { MovaConfig } from "../../src/client.js";

const cfg: MovaConfig = { apiKey: "test", baseUrl: "http://invalid.test", llmKey: "", llmModel: "" };

function makeFetcher(response: unknown, calls: string[]): StatusFetcher {
...
test("CFV-3: assertNotHumanGate allows AI_ATOMIC step_complete", async () => {
  const fetcher = makeFetcher(
    { current_step: { step_id: "ai_step", execution_mode: "AI_ATOMIC" } },
    [],
  );
...
test("CFV-3: guard returns null when reported step_id does not match current_step", async () => {
  ...
  const fetcher = makeFetcher(
    { current_step: { step_id: "different_step", execution_mode: "HUMAN_GATE" } },
    [],
  );
...
test("CFV-3: guard fails open when status fetch throws", async () => {
  const fetcher: StatusFetcher = async () => { throw new Error("network down"); };
