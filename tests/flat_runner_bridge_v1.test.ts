...
test("flat-runner canonical repo drives one real bridge run end-to-end", async () => {
  ...
  const submitted = await movaSubmitDecision(
    ...
    started.contract_id,
    "reject",
    "Rejected through canonical flat-runner bridge.",
  );
  ...
  const persistedBeforeReentry = JSON.parse(await fs.readFile(stateFile, "utf8")) as Record<string, unknown>;
  const secondResume = await movaResumeContract(LOCAL_SEAM_CONFIG, started.contract_id);
  const persistedAfterReentry = JSON.parse(await fs.readFile(stateFile, "utf8")) as Record<string, unknown>;
