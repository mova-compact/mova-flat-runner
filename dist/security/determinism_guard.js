...
function isLikelyRecentEpochMs(n, now) {
...
function isLikelyRecentEpochSec(n, now) {
    if (!Number.isFinite(n))
        return false;
    if (n <= 0)
        return false;
...
export function findDeterminismViolations(output, now = Date.now()) {
    const out = [];
 ...
 */
export function assertDeterministicOutput(executionMode, output, stepId, requestId, now = Date.now()) {
    ...
    if (violations.length === 0)
        return null;
    return flatErr(ERR.NON_DETERMINISTIC_OUTPUT, `Step "${stepId}" is DETERMINISTIC but the supplied output contains likely non-deterministic content (${violations.length} indicator(s)). ` +
        "DETERMINISTIC steps must produce output reproducible from inputs alone.", {
        step_id: stepId,
        execution_mode: "DETERMINISTIC",
        violations,
        remediation: "If the value is genuinely needed, change the step to AI_ATOMIC or move the timestamp/uuid to step inputs so it becomes part of the deterministic seed.",
