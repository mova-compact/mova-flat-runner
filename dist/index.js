...
function cfgBase() {
    ...
    if (!apiKey)
        throw new Error(JSON.stringify(flatErr(ERR.CONFIG_MISSING, "MOVA_API_KEY environment variable is not set.")));
    return {
        apiKey,
...
function cfgFull() {
    ...
    if (!llmKey)
        throw new Error(JSON.stringify(flatErr(ERR.CONFIG_MISSING, "LLM_KEY is required for contract execution (mova_run).")));
...
function normalizeError(e, requestId) {
    ...
    return JSON.stringify(flatErr(code, msg, undefined, retryable, requestId));
}
async function executeTool(name, args) {
    ...
    switch (name) {
        ...
        case "mova_run": {
            ...
            if (!manifest) {
                return JSON.stringify(flatErr(ERR.UNKNOWN_CONTRACT_TYPE, `Unknown contract_type "${contractType}". For custom contracts use mova_contract action=run.`, { available: Object.keys(CONTRACT_MANIFESTS) }, false, requestId));
            }
            // Flow shape guard — verify manifest before sending anything to backend
            const flowCheck = validateFlowShape(manifest.steps);
            if (!flowCheck.ok) {
            ...
            if (!dsCheck.ok) {
                return JSON.stringify(flatErr(ERR.LOCAL_VALIDATION_FAILED, `Input validation failed for contract_type "${contractType}".`, { errors: dsCheck.errors, hint: `Read mova://contracts/${contractType}/manifest for the full DataSpec.` }, false, requestId));
            }
            let config;
            try {
                config = cfgFull();
        ...
        case "mova_decide": {
            ...
            const validOptions = (dp.options ?? []);
            if (validOptions.length > 0 && !validOptions.some(o => o.option_id === option)) {
                return JSON.stringify(flatErr(ERR.LOCAL_INVALID_DECISION_OPTION, `Option "${option}" is not valid for this decision gate.`, { valid_options: validOptions.map(o => o.option_id), provided: option }, false, requestId));
            }
            try {
                ...
                if (!result.ok)
                    return JSON.stringify(flatErr(ERR.API_REQUEST_FAILED, "Decision submission failed", result, false, requestId));
        ...
        case "mova_registry": {
            ...
            const q = qs({ keyword: args.keyword });
            try {
                switch (args.scope) {
                    case "contracts":
                        try {
        ...
        case "mova_contract": {
            ...
            let config;
            try {
                config = needsLlm ? cfgFull() : cfgBase();
            }
            catch (e) {
                return normalizeError(e, requestId);
            }
        ...
        case "mova_health": {
            ...
            try {
                const h = await movaGet(config, "/health");
...
function buildServer() {
    const srv = new Server({ name: "mova-mcp", version: RUNNER_VERSION }, { capabilities: { tools: {}, resources: {} } });
