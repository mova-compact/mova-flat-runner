...
export interface DsField {
  ...
  format?:     string;           // uri, date, iso-alpha-2, ...
  required:    boolean;
  description: string;
  enum?:       string[];
...
function aiStep(stepId: string, title: string, nextStepId: string, systemPrompt: string, model = "openai/gpt-4o-mini") {
...
function deterministicStep(stepId: string, title: string, nextStepId: string, fn: string) {
  return { step_id: stepId, step_type: "deterministic", title, next_step_id: nextStepId, config: { function: fn } };
}

function verifyStep(title: string) {
  return { step_id: "verify", step_type: "verification", title, next_step_id: "decide", config: { recommended_action: "review" } };
}

function decideStep(title: string, decisionKind: string, question: string, options: Array<{ option_id: string; label: string }>) {
...
export const CONTRACT_MANIFESTS: Record<string, ContractManifest> = {
  ...
  invoice: {
    ...
    steps: [
      aiStep("analyze", "OCR Extract Invoice Fields", "verify",
        "You are an invoice OCR agent. The user message contains the invoice image. Extract all fields. Return ONLY a JSON object with: document_id, vendor_name, vendor_iban, vendor_tax_id, total_amount (number), currency (ISO-4217), invoice_date (ISO-8601), due_date (ISO-8601), po_reference (null if missing), subtotal (number), tax_amount (number), line_items (array of {description, quantity, unit_price, amount}), vendor_status (known/unknown/blocked), po_match (matched/partial/not_found), duplicate_flag (bool), ocr_confidence (0.0-1.0), findings (array of {code, severity, summary}).",
  ...
  po: {
    ...
    steps: [
      aiStep("analyze", "PO Risk Analysis", "verify",
  ...
  trade: {
    ...
    steps: [
      aiStep("analyze", "Trade Risk Analysis", "verify",
  ...
  aml: {
    ...
    steps: [
      aiStep("analyze", "AML Alert Triage Analysis", "verify",
  ...
  complaint: {
    ...
    steps: [
      aiStep("analyze", "Complaint Classification & Risk Analysis", "verify",
  ...
  compliance: {
    ...
    steps: [
      aiStep("analyze", "Compliance Rules Check", "verify",
  ...
  credit: {
    ...
    steps: [
      aiStep("analyze", "Credit Risk Scoring", "verify",
  ...
  supply_chain: {
    ...
    steps: [
      aiStep("analyze", "Supplier Screening & Risk Analysis", "verify",
  ...
  churn: {
    ...
    steps: [
      aiStep("analyze", "Churn Risk Prediction", "verify",
  ...
  contract_gen: {
    ...
    steps: [
      aiStep("analyze", "Legal Document Draft Generation", "verify",
