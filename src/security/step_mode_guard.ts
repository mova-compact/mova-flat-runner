...
export function findStepModeViolations(flow: unknown): ModeViolation[] {
  const out: ModeViolation[] = [];
  for (const step of walkSteps(flow)) {
    const id   = asStr(step.id);
    const mode = asStr(step.execution_mode);

    if (mode == null) continue; // step shape problems are graph_guard's job
...
export function assertStepModesValid(
  ...
  return flatErr(
    ...
    `Step execution_mode does not agree with content fields: ${violations.length} violation(s) — ${violations.map((v) => v.kind).join(", ")}.`,
    {
      violations,
