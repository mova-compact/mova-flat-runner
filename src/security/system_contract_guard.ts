...
const DEFAULT_SYSTEM_CONTRACTS: readonly string[] = Object.freeze([
...
export function loadSystemContractAllowList(env: NodeJS.ProcessEnv = process.env): Set<string> {
  const raw = env.MOVA_SYSTEM_CONTRACTS;
  if (raw === undefined) return new Set(DEFAULT_SYSTEM_CONTRACTS);
...
interface Flow {
  steps?:          unknown;
  parallel_steps?: unknown;
}

/** Walk every step in a flow (top-level + parallel). Defensive against unknown shapes. */
function* walkSteps(flow: unknown): Generator<FlowStep> {
  if (!flow || typeof flow !== "object") return;
  const f = flow as Flow;
  if (Array.isArray(f.steps)) {
    for (const s of f.steps) if (s && typeof s === "object") yield s as FlowStep;
  }
  if (Array.isArray(f.parallel_steps)) {
    for (const s of f.parallel_steps) if (s && typeof s === "object") yield s as FlowStep;
  }
}
...
export function findSystemContractViolations(
  flow:        unknown,
  allowList:   Set<string> = loadSystemContractAllowList(),
): SystemContractViolation[] {
  const violations: SystemContractViolation[] = [];
  for (const step of walkSteps(flow)) {
    ...
    if (mode !== "CONTRACT_CALL" || !cid) continue;
 ...
 */
export function assertNoSystemContractCalls(
  flow:      unknown,
  requestId: string,
  allowList: Set<string> = loadSystemContractAllowList(),
  ...
  return flatErr(
    ERR.SYSTEM_CONTRACT_NOT_INVOKABLE,
    `User flow contains CONTRACT_CALL step(s) targeting system contract(s): ${violations.map((v) => v.contract_id).join(", ")}. ` +
