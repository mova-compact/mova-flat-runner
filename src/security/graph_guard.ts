...
interface NextRef {
  // string form: outcome -> step_id;  object form: outcome -> { terminal: "..." }
  [outcome: string]: string | { terminal?: unknown } | undefined;
}
...
export type GraphViolationKind =
  | "size_limit"
  | "self_loop"
  | "dangling_next"
  | "cycle"
...
function outgoingEdges(step: FlowStep): Array<{ outcome: string; target: string }> {
  const next = step.next;
  if (!next || typeof next !== "object") return [];
  const out: Array<{ outcome: string; target: string }> = [];
  for (const [outcome, ref] of Object.entries(next as NextRef)) {
...
export function validateFlowGraph(
  flow:     unknown,
  maxSteps: number = loadMaxSteps(),
): GraphViolation[] {
  if (!flow || typeof flow !== "object") return [];
  const f = flow as Flow;
  const steps = Array.isArray(f.steps) ? (f.steps as FlowStep[]) : [];
  const parallel = Array.isArray(f.parallel_steps) ? (f.parallel_steps as FlowStep[]) : [];
  ...
  function visit(u: string): void {
    ...
    if (step) {
      for (const { target } of outgoingEdges(step)) {
        ...
        if (target === u) continue; // self-loop already reported separately
 ...
 */
export function assertFlowGraphValid(
  flow:      unknown,
  requestId: string,
  maxSteps:  number = loadMaxSteps(),
  ...
  return flatErr(
    ERR.FLOW_GRAPH_INVALID,
    `Flow graph invalid: ${violations.length} violation(s) — ${violations.map((v) => v.kind).join(", ")}.`,
    {
      violations,
      max_steps: maxSteps,
