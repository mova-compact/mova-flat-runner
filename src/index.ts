...
async function executeTool(name: string, args: Args): Promise<string> {
  ...
  switch (name) {
    ...
    case "mova_run": {
      ...
      const missing = manifest.dataspec.inputs.filter(f =>
        f.required && (inputs[f.field] === undefined || inputs[f.field] === null || inputs[f.field] === "")
      );
      if (missing.length > 0) {
        return JSON.stringify({
          ok: false,
      ...
      return JSON.stringify(await movaRunSteps(config, cid, (manifest.validators ?? []) as import("./client.js").ValidatorDef[]));
    }
      ...
      switch (args.view) {
        ...
        case "audit_compact": {
          const res = await fetch(
            `${config.baseUrl.replace(/\/$/, "")}/api/v1/contracts/${cid}/audit/compact/sidecar.jsonl`,
