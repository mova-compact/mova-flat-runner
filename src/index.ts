...
function cfg(): MovaConfig {
  ...
  if (!apiKey) throw new Error("MOVA_API_KEY environment variable is not set.");
  if (!llmKey) throw new Error("LLM_KEY environment variable is not set.");
  return {
    apiKey,
...
const CONTRACT_SCHEMAS: Record<string, Array<{ field: string; question: string; example: string; required: boolean }>> = {
  invoice: [
...
async function executeTool(name: string, args: Args): Promise<string> {
  ...
  async function startContract(contractId: string, templateId: string, policyRef: string, inputs: Array<{key: string; value: string}>, steps: unknown[]) {
  ...
  switch (name) {
    case "mova_hitl_start": {
      const docId = (args.document_id as string | undefined) || `INV-${shortId().toUpperCase()}`;
      ...
      return JSON.stringify(await startContract(cid, "tpl.finance.invoice_ocr_hitl_v0", "policy.hitl.finance.invoice_ocr_v0", [
        { key: "document_id", value: docId }, { key: "document_type", value: "invoice" }, { key: "file_url", value: args.file_url as string },
      ], INVOICE_STEPS));
    }
    case "mova_hitl_start_po": {
      ...
      return JSON.stringify(await startContract(cid, "tpl.erp.po_approval_hitl_v0", "policy.hitl.erp.po_approval_v0", [
        { key: "po_id", value: args.po_id as string }, { key: "approver_employee_id", value: args.approver_employee_id as string },
      ], PO_STEPS));
    }
    case "mova_hitl_start_trade": {
      ...
      return JSON.stringify(await startContract(cid, "tpl.crypto.trade_review_hitl_v0", "policy.hitl.crypto.trade_review_v0", [
        { key: "trade_id", value: args.trade_id as string }, { key: "wallet_address", value: args.wallet_address as string },
        { key: "chain", value: args.chain as string }, { key: "token_pair", value: args.token_pair as string },
        { key: "side", value: args.side as string }, { key: "order_type", value: args.order_type as string },
    ...
    case "mova_hitl_start_aml": {
      ...
      return JSON.stringify(await startContract(cid, "tpl.aml.alert_triage_hitl_v0", "policy.hitl.aml.alert_triage_v0", [
        { key: "alert_id", value: args.alert_id as string }, { key: "rule_id", value: args.rule_id as string },
        { key: "rule_description", value: args.rule_description as string }, { key: "risk_score", value: String(args.risk_score) },
        { key: "customer_id", value: args.customer_id as string }, { key: "customer_name", value: args.customer_name as string },
        { key: "customer_risk_rating", value: args.customer_risk_rating as string }, { key: "customer_type", value: args.customer_type as string },
        { key: "customer_jurisdiction", value: args.customer_jurisdiction as string },
    ...
    case "mova_hitl_start_complaint": {
      ...
      return JSON.stringify(await startContract(cid, "tpl.complaints.handler_hitl_v0", "policy.hitl.complaints.handler_v0", [
        { key: "complaint_id", value: args.complaint_id as string }, { key: "customer_id", value: args.customer_id as string },
        { key: "complaint_text", value: args.complaint_text as string }, { key: "channel", value: args.channel as string },
        { key: "product_category", value: args.product_category as string }, { key: "complaint_date", value: args.complaint_date as string },
        { key: "previous_complaints", value: JSON.stringify(args.previous_complaints ?? []) },
        { key: "customer_segment", value: (args.customer_segment as string | undefined) ?? "" },
        { key: "preferred_language", value: (args.preferred_language as string | undefined) ?? "en" },
      ], COMPLAINTS_STEPS));
    }
    case "mova_hitl_start_compliance": {
      ...
      return JSON.stringify(await startContract(cid, "tpl.compliance.audit_hitl_v0", "policy.hitl.compliance.audit_v0", [
        { key: "document_url", value: args.document_url as string }, { key: "framework", value: args.framework as string },
        { key: "org_name", value: args.org_name as string }, { key: "document_id", value: (args.document_id as string | undefined) ?? `DOC-${shortId()}` },
      ], COMPLIANCE_STEPS));
    }
    case "mova_hitl_start_credit": {
      ...
      return JSON.stringify(await startContract(cid, "tpl.credit.scoring_hitl_v0", "policy.hitl.credit.scoring_v0", [
        { key: "applicant_id", value: args.applicant_id as string }, { key: "monthly_income", value: String(args.monthly_income) },
        ...
        { key: "loan_purpose", value: args.loan_purpose as string },
      ], CREDIT_STEPS));
    }
    case "mova_hitl_start_supply_chain": {
      ...
      return JSON.stringify(await startContract(cid, "tpl.supply_chain.risk_hitl_v0", "policy.hitl.supply_chain.risk_v0", [
        { key: "suppliers", value: JSON.stringify(args.suppliers) }, { key: "category", value: args.category as string },
        { key: "requestor_id", value: args.requestor_id as string },
      ], SUPPLY_CHAIN_STEPS));
    }
    case "mova_hitl_start_churn": {
      ...
      return JSON.stringify(await startContract(cid, "tpl.churn.prediction_hitl_v0", "policy.hitl.churn.prediction_v0", [
        { key: "segment_id", value: args.segment_id as string }, { key: "period_days", value: String(args.period_days) },
        { key: "threshold", value: String(args.threshold) }, { key: "requestor_id", value: args.requestor_id as string },
      ], CHURN_STEPS));
    }
    case "mova_hitl_start_contract_gen": {
      const cid = `ctr-cng-${shortId()}`;
      return JSON.stringify(await startContract(cid, (args.template_id as string | undefined) ?? `tpl.legal.${args.doc_type}_hitl_v0`, "policy.hitl.legal.contract_gen_v0", [
        { key: "doc_type", value: args.doc_type as string }, { key: "party_a", value: args.party_a as string },
        { key: "party_b", value: args.party_b as string }, { key: "jurisdiction", value: args.jurisdiction as string },
        { key: "effective_date", value: args.effective_date as string }, { key: "terms", value: JSON.stringify(args.terms ?? {}) },
      ], CONTRACT_GEN_STEPS));
    }
    case "mova_hitl_decide": {
      ...
      const result = await movaPost(config, `/api/v1/contracts/${cid}/decision`, {
        envelope: {
          ...
          payload: { selected_option_id: args.option, selection_reason: (args.reason as string | undefined) ?? "decision via MOVA MCP" },
        },
    ...
    case "mova_hitl_audit_compact": {
      const res = await fetch(
        `${config.baseUrl.replace(/\/$/, "")}/api/v1/contracts/${args.contract_id}/audit/compact/sidecar.jsonl`,
        { headers: { Authorization: `Bearer ${config.apiKey}`, "X-LLM-Key": config.llmKey, "X-LLM-Model": config.llmModel } }
      );
      return JSON.stringify({ ok: res.ok, status: res.status, journal: await res.text() });
    }
    case "mova_calibrate_intent": {
      const schema = CONTRACT_SCHEMAS[args.contract_type as string];
      if (!schema) return JSON.stringify({ status: "UNKNOWN_CONTRACT_TYPE", message: `Unknown: "${args.contract_type}". Available: ${Object.keys(CONTRACT_SCHEMAS).join(", ")}` });
      const answersMap = new Map<string, string>((args.answers as Array<{field: string; value: string}>).map(a => [a.field, a.value.trim()]));
      ...
      for (const f of schema) { const v = answersMap.get(f.field); if (v) resolved[f.field] = v; }
      return JSON.stringify({ status: "VALID", contract_type: args.contract_type, resolved_inputs: resolved, next_tool: START_TOOL[args.contract_type as string], instruction: `All inputs collected. Call ${START_TOOL[args.contract_type as string]} with these resolved_inputs.` });
    }
    case "mova_list_connectors": {
      const data = await movaGet(config, "/api/v1/connectors") as { connectors: Array<{connector_id: string; display_name: string; description: string}> };
    ...
    case "mova_discover_contracts": {
      ...
      if (args.execution_mode) params.set("execution_mode", args.execution_mode as string);
    ...
    case "mova_list_my_contracts":
      return JSON.stringify(await movaGet(config, args.keyword ? `/api/v1/contracts/mine?keyword=${encodeURIComponent(args.keyword as string)}` : "/api/v1/contracts/mine"));
