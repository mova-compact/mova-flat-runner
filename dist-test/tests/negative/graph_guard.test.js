...
import { assertFlowGraphValid, validateFlowGraph, loadMaxSteps, } from "../../src/security/graph_guard.js";
...
test("CFV-4: loadMaxSteps respects env var", () => {
    assert.equal(loadMaxSteps({ MOVA_FLOW_MAX_STEPS: "50" }), 50);
    assert.equal(loadMaxSteps({}), 200);
    assert.equal(loadMaxSteps({ MOVA_FLOW_MAX_STEPS: "" }), 200);
    assert.equal(loadMaxSteps({ MOVA_FLOW_MAX_STEPS: "garbage" }), 200);
});
