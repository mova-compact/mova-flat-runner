...
export const FORBIDDEN_FLOW_KEYS = Object.freeze([
    "class_definition",
...
export function findInlineClassDefinitionFields(flow) {
 ...
 */
export function assertNoInlineClassDefinition(flow, requestId) {
    const found = findInlineClassDefinitionFields(flow);
    if (found.length === 0)
        return null;
    return flatErr(ERR.INLINE_CLASS_DEFINITION_FORBIDDEN, `Flow body contains inline class-definition field(s): ${found.join(", ")}. ` +
